...
var ControllerScript:AndroidControls;									// Current tap count
...
function Start()
{
	...
	ControllerScript=GameObject.FindWithTag("Controller").GetComponent(AndroidControls);
	
	// Store the default rect for the gui, so we can snap back to it
	guiTexture.pixelInset.width=buttonWidth;
	guiTexture.pixelInset.height=buttonHeight;
	guiTexture.pixelInset.x=-buttonWidth*1.2;
	guiTexture.pixelInset.y=buttonWidth*0.2;
...
function Update()
{	
//	if(position.y<(-0.1) || position.y>(0.1)) //keelab näpu libistamise nupul suuremas ulatuses	
//	{
//		ControllerScript.downArrowPressed=false;
//	}
	...
	{
		tapCount = 0;
		//ControllerScript.leftArrowPressed=false;
	}
	...
	else
	{
		for(var i : int = 0;i < count; i++)
		{
			...
			if ( shouldLatchFinger && ( lastFingerId == -1 || lastFingerId != touch.fingerId ) )
			{
				...
				if ( touchPad ) //TOUCH DETECTED
				{
					gui.color.a = 1;
					ControllerScript.downArrowPressed=true;
